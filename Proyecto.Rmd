...
"a16_i","dpto","b3_i"),
Descripcion = c("Edad","Condicion Juridica","Nivel de Educacion","Cantidad de estudios de I+D realizados de 1 a 8","Tipo de energia utilizada","Departamento donde se ubica la propiedad","Tenencia de Tierras"))
xtable(datos_1)
```
...
```{r}
sp_depto <- readOGR(here("Datos"
,
"ine_depto.shp"))
dframe_depto <- ggplot2::fortify(sp_depto)
dframe_depto <-rename(dframe_depto, dpto = id )
dframe_depto %>%
filter(dpto!=15) %>%
ggplot(aes(x = long, y = lat, group = group, fill = dpto)) +
