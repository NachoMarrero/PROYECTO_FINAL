...
Continuando con la variable de educación, y manteniendo la reagrupación anterior, investigamos como es la relación entre ésta y las actividades de innovación realizadas en el período (siendo 8 el máximo número posible de distintas innovaciones), en el cual observamos que las personas con mayor nivel educativo son mas propensas a realizar innovaciones tanto en ovejas como en vacas. 
...
```{r, fig.cap="Gráfico de barras apiladas que refiere a la realización de innovaciones en Vacas según el nivel educativo del propietario ."} 
...
datos %>% 
  ...
  summarise(n=n()) %>%
  mutate(freq=n/sum(n)) %>%
  ggplot(aes(x=factor(aiv_nueva),y=freq,fill=factor(ba_1)))+
  geom_bar(stat="identity",position = "dodge")+
  labs(x="Innovación",y="Frecuencia")+
...
datos %>% 
  ...
  summarise(n=n()) %>%
  mutate(freq=n/sum(n)) %>%
  ggplot(aes(x=factor(aio_nueva),y=freq,fill=factor(ba_1)))+
  geom_bar(stat="identity",position = "dodge")+
  labs(x="Innovación",y="Frecuencia")+
  scale_fill_brewer(name="Bosques artificiales",palette = "Set3",labels = c(
                                                       '1' = 'Si',
...
datos %>% 
  ...
  summarise(n=n()) %>%
  mutate(freq=n/sum(n)) %>%
  ggplot(aes(x=factor(aiv_nueva),y=freq,fill=factor(pa_1)))+
  geom_bar(stat="identity",position = "dodge")+
  labs(x="Innovación",y="Frecuencia")+
...
datos %>% 
  ...
  summarise(n=n()) %>%
  mutate(freq=n/sum(n)) %>%
  ggplot(aes(x=factor(aio_nueva),y=freq,fill=factor(pa_1)))+
  geom_bar(stat="identity",position = "dodge")+
  labs(x="Innovación",y="Frecuencia")+
  scale_fill_brewer(name="Praderas artificiales",palette = "Set1",labels = c(
                                                       '1' = 'Si',
...
datos %>% 
  ...
  theme_bw()
```
Para utilizar la superficie se corrigió por la calidad del suelo, llevandola a hectáreas conneat 100 equivalentes de forma tal que sean calidades similares en términos de productividad.


```{r fig.cap="Unidades Ganaderas segun la extension del territorio",eval=FALSE}

datos %>% 
  group_by(a,est) %>% 
  mutate(b3_7 = case_when(a %in% c(1:200)  ~ "0-200",
                          a %in% c(201:400)  ~ "200-400",
                          a %in% c(401:600)  ~ "400-600",
                          a > 600 ~ "600")) %>%
  ggplot() +
...
datos %>% 
  ...
  scale_x_discrete(name='')+
          ...
          theme(aspect.ratio=1)
```
Centrandonos en el sueldo por jornal, intentamos observar donde se agrupan mayoritariamente el sueldo por jornal con un boxplot.Vemos que el sueldo por jornal de las personas zafrales se comprende entre menos de 1000$, y varios outliers
