...
```
 ...
 Para realizar la exploración, nos planteamos algunas preguntas para orientar el análisis. En primer lugar, decidimos investigar la condicion Juridica de los propietarios encuestados.
 
```{r,echo=TRUE, fig.cap= "Podemos observar como la mayor cantidad de encuestados son personas fisicas"}
datos %>% 
  group_by(a9) %>% 
  ...
  scale_x_discrete(name="Condición jurídica",labels = c(
                                                       ...
                                                       '4' = 'Otra'))
```
Siguiendo con el analisis de las personas encuestadas, avergiuamos la distribucion de edades y el sexo de las personas fisicas. Tambien analizamos el nivel de educacion de los encuestmaos y nos encontramos con una poblacion muy bien distribuida en los 3 grupos basico, basico superior y terciario, ya que la diferencia entre estos era muy pequeña, lo que nos lleva a pensar que no hay una relacion entre la tenencia de tierra y el nivel de educacion.

```{r fig.cap="Notamos que los propietarios son mayormente hombres de entre 40 y 60 años, e incluso tambien de 60 a 80, lo cual demuestra como la tierra esta mayormente en propiedad de personas adultas y adultos mayore."}
datos %>% filter(a9!=0,a10!=0,a10>0) %>% 
  ...
  scale_fill_brewer(name="Sexo",palette="Dark2",labels=c('1'='M',
                                         '2'='F'))
```
Continuando con la variable de educacion, investigamos como es la relacion entre el esta y la posibilidad de que haya realizado actividades de innovacion en el periodo. Tambien analisamos si varia la posibilidad de contratar personal profesional paralas innovaciones segun la cantidad de innovaciones realizadas.
...
```{r, fig.cap="Observamos una gran correlacion entre estas variblaes, lo cual quiere decir que es mas problable que una persona con mayor grado de estudios innove mas en su produccion que alguien von un menor grado de estudio", eval=FALSE} 
datos %>% 
