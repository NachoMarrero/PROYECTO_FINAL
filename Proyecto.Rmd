...
datos %>% 
  ...
  summarise(n=n()) %>%
  mutate(freq=n/sum(n)) %>%
  ggplot(aes(x=factor(aiv_nueva),y=freq,fill=factor(bn_3)))+
  geom_bar(stat="identity",position = "dodge")+
  labs(x="Innovación",y="Frecuencia")+
  scale_fill_brewer(name="Bosques naturales",palette = "Set3",labels = c(
                                                       '1' = 'Si',
                                                       '2' = 'No'))+ theme_bw()
```
...
datos %>% 
  ...
  summarise(n=n()) %>%
  mutate(freq=n/sum(n)) %>%
  ggplot(aes(x=factor(aio_nueva),y=freq,fill=factor(bn_3)))+
  geom_bar(stat="identity",position = "dodge")+
  labs(x="Innovación",y="Frecuencia")+
  scale_fill_brewer(name="Bosques naturales",palette = "Set3",labels = c(
                                                       '1' = 'Si',
