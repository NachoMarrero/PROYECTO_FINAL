...
```{r, fig.cap= "Gráfico de barras sobre la condicion Jurídica de los encuestados"}
...
```{r fig.cap="Gráfico de barras correspondiente a la relación entre edades y sexo de las personas físicas encuestadas."}
...
```{r, fig.cap="Gráfico de barras apiladas que refiere a la realización de innovaciones en establecimientos con vacunos según el nivel educativo del propietario"} 
...
```{r fig.cap="Gráfico de barras apiladas que refiere a la realización de innovaciones en establecimientos con ovinos según el nivel educativo del propietario"}
...
```
  ...
  También analizamos la relación entre la contratación de asistencia técnica especializada y las innovaciones, podemos visualizar como la cantidad de personal profesional contratado es mayor en quienes mas innovaciones realizaron, podriamos deducir que quienes mas innovan tambien poseen mayores niveles de asistencia profesional para dicha actividad, tanto en vacunos como en ovinos.
...
datos %>% 
  ...
  mutate(aiv_nueva = case_when(aiv_nueva %in% c(0)  ~ "0 nin",
   ...
   mutate(est = case_when(est %in% c(0)  ~ "0 nin",
                          est %in% c(1,2) ~ "< 149 UG",
                          est %in% c(3,4) ~ "150 a 599 UG",
                          est %in% c(5,6) ~ "600 a 1.999 UG",
                          est %in% c(7,8) ~ " > 2.000 UG")) %>% 
  filter(a9=="1") %>% 
  summarise(n=n() ) %>%
  mutate(freq=n/sum(n)) %>%
...
```{r,  fig.cap="Unidades Ganaderas segun la extension del territorio"}
datos %>% 
        group_by(b3_7,est) %>%
...
```
  ...
  A continuación observaremos la proporción de encuestados que contratan personal profesional y/o personal zafral según el tipo de condición jurídica de la empresa. Se observa que mayor proporcion de los encuestados contratan personal profesional que zafral.

```{r,fig.cap="Diagrama de barras que compara la cantidad de personal profesional y zafral contratado."}
...
Cant_CJ<- datos %>%
        ...
        ggplot(tabla_porcentaje)+
          ...
          scale_y_continuous()+
          scale_fill_discrete(labels=c('Persona Fisica','Sociedad sin contrato','Sociedad con contrato','Otra'))+
          labs(y='Porcentaje Personal zafral',fill='',x='')+
          scale_y_continuous(breaks = c(20,40,60,80))+
          theme(aspect.ratio=1)+
        theme_bw()
```

Por otra parte, nos interesó averiguar si podiamos encontrar alguna diferencia sobre la renumeración por jornal que perciben los contratados para zafra, según su Condición Jurídica. Observamos que no hay mucha diferencia entre sus cuartiles y mediana en las 3 primeras condiciones, cercanas a los 1000$ por jornal, pero si lo hay en la Condición Jurídica otra la cual tiene sueldo por jornal algunos muy bajos, datos atipicos. 

```{r,fig.cap="Remuneracion por Jornal segun condicion juridica del encuestado."}
...
```{r, fig.cap="Sueldo por jornal trabajado."}

datos %>% 
  filter(pz_1==1,b3_7>100,pz_2!=0,pz_3!=0) %>% 
  summarise('Sueldo_por_jornal'=pz_3/pz_2) %>% 
  ggplot()+geom_boxplot(fill = "#4271AE",aes(y=Sueldo_por_jornal))+labs(y='Sueldo por jornal')+
         coord_flip()+
        theme_bw()+theme(aspect.ratio=0.2)+ theme(axis.ticks = element_blank(), 
                 axis.text.y = element_blank())
```

  Filtrando por quienes contratan persona zafral, queriamos ver si hay alguna relación entre los jornales y su respectiva renumeración.Observamos clara relacion lineal fuerte negativa entre los jornales y su respectiva renumeración. Cuanto más jornales realicen, mayor será su renumeración
